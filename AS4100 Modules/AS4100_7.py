...
def s7_1_Tension_Capacity(A_g, A_n, f_y, f_u, k_t, φ = 0.9, 
                          ultimate_uncertainty = 0.85):
    '''
    Calculates the tension capacity of a section according to AS4100 S7.1.
    
    A_g: gross section area
    A_n: net section area
    f_y: yield strength. If different components have different yield
         strengths the minimum strength of the section should be used.
         Where a section has a significantly different strength the result
         may be too conservative and this function may not be appropriate
         (i.e. a 250 grade web and a 450 grade flange).
    f_u: ultimate strength
    k_t: connection efficiency factor.
    φ: Capacity reduction factor, default is 0.9
    ultimate_uncertainty: a factor for uncertainty about the ultimate
